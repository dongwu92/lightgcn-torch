...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
        ...
        self.agg_weight = torch.nn.Parameter(torch.FloatTensor(self.latent_dim, self.latent_dim))
        ...
        for layer in range(self.n_layers):
            if self.A_split:
                ...
                for f in range(len(g_droped)):
                    agg = torch.sparse.mm(g_droped[f], all_emb)
                    nhid = torch.mm(agg, self.agg_weight)
                    temp_emb.append(nhid)
                side_emb = torch.cat(temp_emb, dim=0)
                all_emb = side_emb
            else:
                all_emb = torch.sparse.mm(g_droped, all_emb)
                all_emb = torch.mm(all_emb, self.agg_weight)
            embs.append(all_emb)
