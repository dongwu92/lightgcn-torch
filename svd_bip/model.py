...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
    ...
    def bpr_loss(self, users, pos, neg):
        ...
        pos_svds = torch.mul(userEmb0, posEmb0)
        pos_svds = torch.sum(pos_svds, dim=1)
        neg_svds = torch.mul(userEmb0, negEmb0)
        neg_svds = torch.sum(neg_svds, dim=1)
        loss += 1e-2 * torch.mean(torch.nn.functional.softplus(neg_svds - pos_svds))
