...
class LightGCN(BasicModel):
    ...
    def __init_weight(self):
        ...
        self.embedding_item = torch.nn.Embedding(
            num_embeddings=self.num_items, embedding_dim=self.latent_dim)
        self.topics = torch.nn.Parameter(torch.FloatTensor(self.num_topics, self.dim_topic)).cuda()
        self.topic_relu = torch.nn.ReLU().cuda()
...
# random normal init seems to be a better choice when lightGCN actually don't use any non-linear activation function
            ...
            nn.init.normal_(self.topics, std=0.1)
    ...
    def computer(self):
        ...
        users_topic_weights = torch.nn.functional.softmax(torch.mm(org_users_emb, self.topics.t()), dim=-1)
        items_topic_weights = torch.nn.functional.softmax(torch.mm(org_items_emb, self.topics.t()), dim=-1)
        users_emb = users_org = torch.cat([self.embedding_user.weight, torch.mm(users_topic_weights, self.topics)], dim=-1)
        items_emb = items_org = torch.cat([self.embedding_item.weight, torch.mm(items_topic_weights, self.topics)], dim=-1)
